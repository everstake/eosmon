...
get_headblock()
{
   BLOCK=`curl --insecure --connect-timeout 6 -s http://127.0.0.1:9999/v1/chain/get_info |jq -r ".head_block_num"`
   if [ -z $BLOCK ]; then
...
get_libblock()
{
   BLOCK=`curl --insecure --connect-timeout 6 -s http://127.0.0.1:9999/v1/chain/get_info |jq -r ".last_irreversible_block_num"`
   if [ -z $BLOCK ]; then
...
get_cpu_usage()
{
   NODEPID=`netstat -tlpn 2>/dev/null | grep "0.0.0.0:9999"|awk '{print $7}'| sed 's/\/nodeos//g'|sed 's/ //g'`
   if [ -z "$NODEPID" ]; then 
      echo ""
   else
...
while true; do
   echo "start"
   get_memsize > ${METRICDIR}/bos_metrics.prom
   get_headblock >> ${METRICDIR}/bos_metrics.prom
   get_libblock >> ${METRICDIR}/bos_metrics.prom
   get_cpu_usage >> ${METRICDIR}/bos_metrics.prom
   get_mem_usage >> ${METRICDIR}/bos_metrics.prom
   sleep 4;
done
